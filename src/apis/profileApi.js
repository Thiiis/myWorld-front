...
//   // axios 인터셉터를 통해 JWT 토큰이 자동으로 헤더에 포함되어야 함
//   return axios.get("/profiles/detail/me");
// }
...
function getAnotherProfileDetail(account) {
  return axios.get(`/profiles/detail/${account}`);
}

/**
 * 프로필 텍스트 정보 업데이트 (인증 필요)
 * @param {object} profileData - 업데이트할 프로필 데이터. 예: { nickname: "새 닉네임", statusMessage: "..." }
 * @returns {Promise} Axios Promise
 */
// function profileUpdate(profileData) {
//   // 백엔드가 @PutMapping과 @RequestBody를 사용하므로 PUT 메소드로 JSON 데이터를 전송
//   return axios.put("/profiles/update", profileData);
// }
...
//   return axios.put("/profiles/update/image", formData, {
//     headers: {
