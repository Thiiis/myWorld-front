...
export default {
  ...
  state: () => ({
    // 검색/정렬/폴더
    searchQuery: "", sortOrder: "asc", activeFolder: "all",
    // 목록
    diaryEntries: [],
    // 선택된 일기 (모달)
    selectedEntry: null,
    // 단일 일기 상세 데이터
    did: "", title: "", content: "", createdAt: "", updatedAt: "", totalPages: 1, totalItems: 0,
  }),
  getters: {
  ...
  actions: {
    ...
    async fetchDiary({ commit }, did) {
      try {
        ...
        commit("setSelectedEntry", { ...res.data, date,});  // selectedEntry에 date 필드 추가
        // 단일 데이터 state에도 반영
    ...
    // 📌 단일 삭제
    async deleteDiary({ commit }, { did, hostAccount = null }) {
      try {
